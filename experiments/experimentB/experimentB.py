...
def printImage(img_name, data, height, width, rgb_img):
    ...
    plt.savefig(f"{img_name}.png", format="png", dpi=600)
    #plt.show()
...
def calcMetrics(test_all_labels, test_all_preds_argmax, test_all_preds_softmax):
    ...
    precision, recall, fscore, support = precision_recall_fscore_support(test_all_labels, test_all_preds_argmax, beta=1.0, average=None, labels=[0,1,2,3])
    precision_w, recall_w, fscore_w, support_w = precision_recall_fscore_support(test_all_labels, test_all_preds_argmax, beta=1.0, average='weighted', labels=[0,1,2,3])

    with open(f'results/data.csv' , mode='a', newline='') as file_csv:
        csv_writer = csv.writer(file_csv)
        csv_writer.writerow([img[0], acc_value, per_class_accuracy, precision, recall, fscore, specificity, roc_value, support, precision_w, recall_w, fscore_w])
...
image_list = [  ('ID0033img.mat', 'ID0033gt.mat'),
...
for i, img in enumerate(image_list):
    train_img_list = image_list[:i] + image_list[i+1:]

    train_data, train_labels, val_data, val_labels, test_data, test_labels = processImage(train_img_list[0][0], train_img_list[0][1], image_patch)
    for im in train_img_list[1:]:
        tdata, tlabels, vdata, vlabels, tst_data, tst_labels = processImage(im[0], im[1], image_patch)
        
        train_data=np.concatenate((train_data, tdata), axis=0)
        train_labels=np.concatenate((train_labels, tlabels), axis=0)
        val_data=np.concatenate((val_data, vdata), axis=0)
        val_labels=np.concatenate((val_labels, vlabels), axis=0)
    ...
    train_data = torch.cat((train_data, val_data), 0)
    train_labels = torch.cat((train_labels, val_labels), 0)
    ...
    testLoader=DataLoader(TensorDataset(test_data,test_labels-1),batch_size=batch_size,shuffle=True) #shuffle=False

    net = HyBrainViT(patchSize=image_patch, nBlocks=5, mlp_dim=4, mode='CAF', numHeads=16, embedDim=64, numClasses=4, dropout=0.1)
    net.to(device) #7 16

    loss_fn = nn.CrossEntropyLoss().cuda()
    optimizer = optim.AdamW(net.parameters(), lr=1e-4, betas=(0.9, 0.999), weight_decay=5e-5) #0.9 0.99
    ...
    for epoch in tqdm(range(epochs)):
        ...
        val_all_labels = []
        with torch.no_grad():
            for _, val_data in enumerate(testLoader, 0): #valLoader
        ...
        avg_val_loss.append(val_loss / len(testLoader)) #valLoader

        if(avg_val_loss[-1] < best_val_loss):
            best_val_loss = avg_val_loss[-1]
            torch.save(net.state_dict(), f'results/models/best_model_test{img}_hs1.pth')

        #if early_stopper.early_stop(val_loss / len(testLoader)):             
        #    break
    ...
    #torch.save(net.state_dict(), f'best_model_test{img}.pth')

    np.save(f'train_loss_test{img}.npy',np.array(avg_train_loss))
    np.save(f'val_loss_test{img}.npy',np.array(avg_val_loss))
    #np.save(f'val_roc_test2.npy',np.array(roc_val))
    ...
    lab_0_indices = np.where(o_labels==0)
    o_data = np.delete(o_data, lab_0_indices, axis=0)
    o_labels = np.delete(o_labels, lab_0_indices)

    testLoader = DataLoader(TensorDataset(torch.from_numpy(o_data.transpose(0,2,1)).type(torch.FloatTensor),torch.from_numpy(o_labels-1).type(torch.LongTensor)),batch_size=batch_size,shuffle=False)

    net = HyBrainViT(patchSize=image_patch, nBlocks=5, mlp_dim=4, mode='CAF', numHeads=16, embedDim=64, numClasses=4, dropout=0.1)
    net.load_state_dict(torch.load(f'results/models/best_model_test{img}_hs1.pth'))
    net.to(device) #7 16
    ...
    with torch.no_grad():
        for _, test_data in enumerate(testLoader, 0): #valLoader
    ...
    with torch.no_grad():
        for _, new_data in enumerate(newLoader, 0):
            new_inputs, _ = new_data[0].to(device, dtype=torch.float32), new_data[1].to(device, dtype=torch.long)
