...
def printImage(img,data, height, width, rgb_img):
    ...
    plt.axis('off')
    plt.savefig(f"{img}.png", format="png", dpi=600)
    plt.show()
...
def calcMetrics(test_all_labels, test_all_preds_argmax, test_all_preds_softmax):
    ...
    specificity = specificity_score(test_all_labels, test_all_preds_argmax, average=None)
...
train_data, train_labels, val_data, val_labels, test_data, test_labels = processImage(image_list[0][0], image_list[0][1], image_patch)
for img in image_list[1:]:
    tdata, tlabels, vdata, vlabels, tst_data, tst_labels = processImage(img[0], img[1], image_patch)
...
#         torch.save(net.state_dict(), f'best_model_test.pth')
...
# np.save(f'train_loss_test.npy',np.array(avg_train_loss))
#np.save(f'val_loss_test.npy',np.array(avg_val_loss))
...
net = HyBrainViT(patchSize=image_patch, nBlocks=5, mlp_dim=4, mode='CAF', numHeads=16, embedDim=64, numClasses=4, dropout=0.1)
net.load_state_dict(torch.load('best_model_test.pth'))
net.to(device) #7 16
...
##############################################
#NEW IMG
def predictImg(img, gt):
    hsi_dataset= h5.loadmat(img)
    hsi_data = hsi_dataset.get('preProcessedImage')
    hsi_data = normal(hsi_data)

    hsi_dataset_labels = h5.loadmat(gt)
    hsi_labels = hsi_dataset_labels.get('groundTruthMap')
    hsi_labels = substituteLabel(hsi_labels)
    ...
    printImage(img,new_all_preds_argmax, hsi_data.shape[0],hsi_data.shape[1], rgb_img)
    #calcMetrics(new_all_labels, new_all_preds_argmax, new_all_preds_softmax)

for image in image_list:
    predictImg(image[0], image[1])
